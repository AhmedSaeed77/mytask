...
<body>
    <center class="fs-1">Edit Article</center>
    @if ($errors->count() > 0)
    ...
    <form id="uploadFile" class="container" method="post" action="{{ route('article.update', $article->id) }}" enctype="multipart/form-data">
        {{ method_field('PUT') }}
        @csrf
        <div class="mb-3">
            <label for="exampleInputEmail1">Title</label>
            <input type="text" class="form-control" value="{{ $article->title }}" aria-describedby="emailHelp" name="title" >
            @if ($errors->any())
